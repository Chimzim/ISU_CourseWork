...
public class PokerHand
{
  ...
  {
    sortNoSuit();
    int cardType1 = cards[0].getRank();
    int cardType2 = cards[cards.length - 1].getRank();
    int card1 = 1;
    int card2 = 1;
    for(int i = 1; i < cards.length - 1; i++) {
    	if(cardType1 == cards[i].getRank()) {
    		card1 += 1; 
    	}
    	if(cardType2 == cards[i].getRank()) {
    		card2 += 1;
    	}
    }
    if(card2 == 3 && card1 == 2) {
  ...
  private void sortNoSuit()
  {
	for(int i = 0; i < cards.length; i++) {
		...
		int minimum = findIndexOfMinNoSuit(i);
