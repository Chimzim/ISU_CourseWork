   ...
   implements ExtendedPriorityQueue<E>
{
  ...
  private void percolateUp() {
	  int parent = (list.size()-2)/2;
	  int current = list.size()-1;
	  while(current > 0 && list.get(current).compareTo(list.get(parent)) < 0) { 
		  swap(parent, current);
		  current = parent;
		  parent = (current-1)/2;
	  }
  ...
  public  E getLastInternal() {
	  ...
	  int parent = list.size()-2;
	 return list.get((parent/2)); 
  }
  ...
  private void percolateDown(int start)
  {
    ...
    int current = start;
    /*Run while there is still a "child" based on the starting point*/
    while(childToSwap < list.size()) {
    	/*If there is a right and left child*/
    	int rightChild = leftChild+1;
    	if(rightChild < list.size()) {
    		/*If the right child is smaller than the left child reset childToSwap to be the right Child*/
    		if(list.get(leftChild).compareTo(list.get(rightChild)) > 0) {
    			childToSwap = rightChild;
    		} 
    	}
    	/*if the start aka current is less than the child to swap then it swaps them*/
    	if(list.get(current).compareTo(list.get(childToSwap)) > 0 ) {
    		swap(current, childToSwap);
    	}
    	/*Moves down, by setting the current aka start to be the child to swap and then the child to swap be the left child */
    	current = childToSwap;
    	childToSwap = (2*current)+1;
    }
  }
