...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int i, smallestArrival; //smallest aka oldest arrival time
	smallestArrival = PageFrames[0].time_of_arrival; //smallest will be set to the first time in the page frame table
	for(i = 0; i < num_frames; i++) {
	  if(PageFrames[i].time_of_arrival < smallestArrival) { //If there is a smaller arrival time set the arrival time to the time from the page frame
	    smallestArrival = PageFrames[i].time_of_arrival;
	    frame_to_evict = i; //the index to evict becomes i 
	  }
	}
...
{
	int frame_to_evict = 0;
	int i, lastUsed; //the last used page frame
	lastUsed = PageFrames[0].time_of_access;  //sets it to the time of access of the first item in the page frame
	for(i = 0; i < num_frames; i++) {
	  if(PageFrames[i].time_of_access < lastUsed) { //If there is an item that was accesses before the current "lastUsed" lastUsed becomes that access time
	    lastUsed = PageFrames[i].time_of_access;
	    frame_to_evict = i; //the index to evict becomes i
	  }
	}
	/*TODO: fill in the code below */
...
{
	int frame_to_evict = 0;
	int i, j, temp = 0, found; //temp is the next time an item will be needed and found determines if it is ever needed again (Pages)
	/*TODO: fill in the code below */
	for(i = 0; i < num_frames; i++) { //Looping through all the items in the page frame
	  found = -1; //resets found to -1 (Not Found) every iteration of the items in the page frame table
	  for(j = current_access+1; j < num_accesses; j++) { //starts at what needs to be in the page frame and loops until the end of the accesses
	    if(PageAccesses[j] == PageFrames[i].page_id) { //If the page will need to be used again set found to 1
	      found= 1;
	      break;
	    }
	  }
	  if(found == -1) { //if it isn't found evict that page 
	    frame_to_evict = i;
	    break;
	  }else {
	    if(temp < j) { // if it is found set the page to evict to be i and then temp to j if J is greater than the current temp
	      frame_to_evict = i;
	      temp = j;
	    }
	  }
