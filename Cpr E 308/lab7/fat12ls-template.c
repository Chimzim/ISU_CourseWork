...
void decodeBootSector(struct BootSector * pBootS, unsigned char buffer[])
{
 int i = 0; 
  ...
  unsigned short iSector = endianSwap(buffer[11], buffer[12]);
  pBootS->iBytesSector = iSector;
  /*Getting the Sectos per cluster*/
  pBootS->iSectorsCluster = buffer[13];
  /*Getting the number of reserved sectors*/
  unsigned short numReserved = endianSwap(buffer[14], buffer[15]);
  pBootS->iReservedSectors = numReserved;
  /*Getting the number of FATs*/
  pBootS->iNumberFATs = buffer[16];
  /*Getting the number of root directory entries*/
  unsigned short numEntries = endianSwap(buffer[17], buffer[18]);
  pBootS->iRootEntries = numEntries;
  /*Getting the number of logical Sectors*/
  unsigned short numLogSectors = endianSwap(buffer[19],buffer[20]);
  pBootS->iLogicalSectors = numLogSectors;
  /*Getting medium desciptor*/
  pBootS->xMediumDescriptor = buffer[21];
  /*Getting the number of sectors per FAT*/
  unsigned short FAT_sector = endianSwap(buffer[22], buffer[23]);
  pBootS->iSectorsFAT = FAT_sector;
  /*Getting the number of sectors per track*/
  unsigned short track_sector = endianSwap(buffer[24], buffer[25]);
  pBootS->iSectorsTrack = track_sector;
  /*Getting the number of heads*/
  unsigned short numHeads = endianSwap(buffer[26], buffer[27]);
  pBootS->iHeads = numHeads;
  /*Getting the number of Hidden sectors*/
  unsigned short hiddenSector = endianSwap(buffer[28], buffer[29]);
  pBootS->iHiddenSectors = hiddenSector;	
}
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + 1)   {
		...
		//Display Attributes
		parseAttributes(string, buffer[i*32+11]);
		printf("%s\t", string);
		
		//Display Time
		unsigned short temp_time = endianSwap(buffer[i*32+22], buffer[i*32+23]);
		parseTime(string, temp_time);
		printf("%s\t", string);
		
		//Display Date
		unsigned short temp_date = endianSwap(buffer[i*32+24], buffer[i*32+25]);
		parseDate(string, temp_date);
		printf("%s\t", string);
		
		//Display Size
		unsigned short temp_size = endianSwap(buffer[i*32+28], buffer[i*32+29]);
		unsigned short temp_size1 = endianSwap(buffer[i*32+30], buffer[i*32+31]);
		unsigned short mySize = temp_size1 * (2*256);
		mySize += temp_size;
		printf("%d\n", mySize);
		//
