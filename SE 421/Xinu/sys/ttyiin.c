...
LOCAL void erase1(struct	tty   *iptr, struct	csr	*cptr);
LOCAL void echoch(char	ch, struct	tty   *iptr, struct	csr	*cptr);
LOCAL void eputc(char	ch, struct	tty   *iptr, struct	csr	*cptr);
	...
	register struct	tty	*iptr;	/* pointer to tty block		*/
{
	...
	if ( (ch=cptr->crbuf) & SLUERMASK)	/* read char from device*/
		return(SYSERR);				/* discard if error	*/
	if (iptr->imode == IMRAW) {
		if (scount(iptr->isem) >= IBUFLEN) {
			return(SYSERR);			/* discard if no space	*/
		}
 ...
 *------------------------------------------------------------------------
 */
LOCAL void echoch(ch, iptr, cptr)
