	...
	public DataDependenceGraph(Graph dfg){
		...
		for(Edge dfEdge : dfg.edges()){
			...
			if(fromStatement == null){
				Log.warning("From node has no parent or is null: " + Integer.toHexString(from.addressBits()));
				continue;
			}
			if(toStatement == null){
				Log.warning("To node has no parent or is null: " + Integer.toHexString(to.addressBits()));
				continue;
			}
